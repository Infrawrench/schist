...
fn fbm(p: vec2<f32>, seed: u32, n: u32) -> f32 {
    var sum = 0.0;
    var amp = 0.5;
    var freq = 1.0;
    var norm = 0.0;
    for (var o = 0u; o < n; o++) {
        sum += value_noise(p * freq, seed + o * 7919u) * amp;
        norm += amp;
        amp *= 0.5;
        freq *= 2.0;
    }
...
fn glass_height(p: vec2<f32>) -> f32 {
    let kind = u32(args[3]);
    if kind == 0u {
        return fbm(p / max(args[2] * 2.0, 1.0), 337u, 3u);
    }
    if kind == 1u {
        let uv = trunc_fract(p / args[4]);
        return (abs(uv.x - 0.5) + abs(uv.y - 0.5)) * 0.7;
    }
    return surface(kind - 2u, p, args[4], 149u);
}

fn texture_height(p: vec2<f32>) -> f32 {
    let value = surface(u32(args[1]), p, args[2], 61u);
    return select(value, 1.0 - value, args[6] != 0.0);
}
...
fn effect(pos: vec2<i32>) -> vec4<f32> {
    ...
    if mode == 0u {
        let xy = p + vec2(0.5);
        let gx = glass_height(xy + vec2(1.0, 0.0)) - glass_height(xy - vec2(1.0, 0.0));
        let gy = glass_height(xy + vec2(0.0, 1.0)) - glass_height(xy - vec2(0.0, 1.0));
        return straight(sample_premul(xy + vec2(gx, gy) * args[1] * 1.2 - vec2(0.5)));
    }
    if mode == 1u {
        let xy = p + vec2(0.5);
        let a = fbm(xy / args[1], 1049u, 2u) - 0.5;
        let b = fbm(xy / args[1] + vec2(31.0, 17.0), 1049u, 2u) - 0.5;
        return straight(sample_premul(xy + vec2(a, b) * args[2] * 2.0 - vec2(0.5)));
    }
    let gx = texture_height(p + vec2(1.0, 0.0)) - texture_height(p - vec2(1.0, 0.0));
    let gy = texture_height(p + vec2(0.0, 1.0)) - texture_height(p - vec2(0.0, 1.0));
    let shade = 1.0 + (gx * args[4] + gy * args[5]) * args[3] * 6.0;
    let original = read_pixel(pos);
    return vec4(clamp(original.rgb * shade, vec3(0.0), vec3(1.0)), original.a);
}
