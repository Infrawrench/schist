...
use crate as camera_sync;
use crate::{CameraSync, Engine, Outcome, Source};
...
pub fn sources() -> Vec<(String, PathBuf)> {
    with_activity(|env, _| {
...
fn credential_path(root: &Path) -> PathBuf {
    ...
    hash.write(crate::CREDENTIAL_KEY.as_bytes());
...
pub fn restore_status(rule: &mut CameraSync) {
...
fn run_job(env: &mut Env<'_>, class: &JClass<'_>, context: &JObject<'_>) -> Result<bool> {
    ...
    let view: schist_app_settings::ViewOptions = match std::fs::read(&prefs) {
...
pub fn prepare_sync<'local>(mut env: EnvUnowned<'local>, _class: JClass<'local>) {
...
pub fn stop_sync<'local>(mut env: EnvUnowned<'local>, _class: JClass<'local>) {
    env.with_env(|_| -> jni::errors::Result<()> {
        CANCEL
            .lock()
            .unwrap_or_else(|e| e.into_inner())
