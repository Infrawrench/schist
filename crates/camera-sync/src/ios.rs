...
use crate::{Candidate, Locate, LIBRARY_ALBUM};
...
pub fn background_credentials(enabled: bool) -> std::result::Result<(), i32> {
    autoreleasepool(|_| unsafe {
        ...
        let _: () =
            msg_send![query, setObject: &*ns(crate::CREDENTIAL_KEY), forKey: kSecAttrServer];
...
pub fn request_authorization(done: impl FnOnce(std::result::Result<(), String>) + Send + 'static) {
    if authorized() {
        done(Ok(()));
        return;
    }
...
pub fn albums() -> Vec<(String, String, u64)> {
    if !authorized() {
        return Vec::new();
    }
    autoreleasepool(|_| unsafe {
...
pub fn list_assets(album: &str) -> Result<Vec<Candidate>> {
    if !authorized() {
...
pub fn take_background_request() -> Option<Arc<AtomicBool>> {
    let mut state = background();
    if !state.requested {
        return None;
    }
...
pub fn finish_background(success: bool) {
...
pub fn run_started(cancel: Arc<AtomicBool>) {
    background().active_cancel = Some(cancel);
}
pub fn run_finished(success: bool) {
