...
fn compute(i: u32) {
    let left = f32(i % shape.width) + args[0];
    let right = left + 1.0;
    let top = args[1] + f32(i / shape.width);
    let edges = arrayLength(&src) / 5u;
    var coverage = 0.0;
    let subsamples = u32(args[3]);
    for (var s = 0u; s < subsamples; s++) {
        let y = top + (f32(s) + 0.5) / f32(subsamples);
        var cursor = left;
        ...
        for (var iteration = 0u; iteration <= edges; iteration++) {
            ...
            for (var e = 0u; e < edges; e++) {
                ...
                if p.x <= cursor {
                    winding += select(i32(p.y), 1, args[2] == 0.0);
                } else if p.x < next {
                    next = p.x;
                }
            }
            if inside(winding) {
                coverage += (next - cursor) / f32(subsamples);
            }
            if next >= right {
                break;
            }
