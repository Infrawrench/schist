...
fn payload(telemetry_id: &str, gpu_info: fn() -> Option<&'static GpuInfo>) -> serde_json::Value {
...
/// One ping.
fn send_telemetry(telemetry_id: &str, gpu_info: fn() -> Option<&'static GpuInfo>) -> bool {
    let body = payload(telemetry_id, gpu_info);
...
fn telemetry_turn(schist_folder: &Path, gpu_info: fn() -> Option<&'static GpuInfo>) {
    ...
    // Try 3 times
    for _ in 0..3 {
        if send_telemetry(&telemetry_id, gpu_info) {
            // This worked!
            return;
        }
...
fn telemetry_loop(schist_folder: PathBuf, gpu_info: fn() -> Option<&'static GpuInfo>) {
    ...
    loop {
        ...
        telemetry_turn(&schist_folder, gpu_info);
...
pub fn start(schist_folder: Option<PathBuf>, gpu_info: fn() -> Option<&'static GpuInfo>) {
    if let Some(schist_folder) = schist_folder {
        let _ = thread::Builder::new()
            .name("telemetry".into())
            .spawn(move || telemetry_loop(schist_folder, gpu_info));
    }
}
