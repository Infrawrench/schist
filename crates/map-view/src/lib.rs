...
use gpui::{point, px, size, Bounds, Context, Pixels, Point, RenderImage};
...
pub fn tile_coords(lat: f64, lon: f64, zoom: i32) -> (f64, f64) {
...
pub fn coords_to_lat_lon(x: f64, y: f64, zoom: i32) -> (f64, f64) {
    let n = 2f64.powi(zoom);
...
impl MapState {
    ...
    pub fn marker_copies(&self, lat: f64, lon: f64) -> Vec<(f32, f32)> {
    ...
    pub fn window_at(&self, lat: f64, lon: f64) -> (f32, f32) {
...
pub fn cluster_markers(points: &[(usize, f32, f32)]) -> Vec<Vec<(usize, f32, f32)>> {
...
impl MapState {
    pub fn prepare_paint(&mut self, bounds: Bounds<Pixels>, scale: f32) -> MapPaint {
        let map = self;
    ...
    pub fn load_tiles<T: 'static>(
        &mut self,
        access: impl Fn(&mut T) -> &mut MapState + Copy + 'static,
        cx: &mut Context<T>,
    ) {
        let map = self;
        ...
        cx.spawn(async move |this, cx| loop {
            ...
            if batch.is_empty() {
                this.update(cx, |ws, _| access(ws).ticker = false).ok();
                return;
            }
            let results = cx
                .background_executor()
                    ...
                    batch
                        ...
                        .map(|(z, x, y)| {
                            let img = fetch_tile(z, x, y).map(with_gutter).and_then(|img| {
                                rgba_to_render_image(img.width(), img.height(), img.into_raw())
                            });
                            ((z, x, y), img)
                        })
                        .collect::<Vec<_>>()
            ...
            let keep = this.update(cx, |ws, cx| {
                for (key, img) in results {
                    ...
                    access(ws).tiles.insert(key, state);
                }
                cx.notify();
            });
            if keep.is_err() {
