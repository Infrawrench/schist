...
fn premul(p: vec4<f32>) -> vec4<f32> { return vec4<f32>(p.rgb * p.a, p.a); }
fn straight(p: vec4<f32>) -> vec4<f32> {
    if p.a > 0.000001 { return vec4<f32>(p.rgb / p.a, p.a); }
...
fn round_away(v: vec2<f32>) -> vec2<f32> { return sign(v) * floor(abs(v) + 0.5); }
...
fn run_effect(@builtin(global_invocation_id) id: vec3<u32>) {
    if id.x >= image.width || id.y >= image.rows { return; }
