...
fn compute(line: u32) {
    ...
    if args[2] != 0.0 {
        ...
        for (var k = 0u; k < min(r, inner); k++) { acc += src[base + k * stride]; }
        let norm = 1.0 / window;
        for (var i = 0u; i < inner; i++) {
            let add = src[base + min(i + r, inner - 1u) * stride];
            var sub = src[base];
            if i > r { sub = src[base + (i - r - 1u) * stride]; }
            acc += add - sub;
            dst[base + i * stride] = acc * norm;
        }
    } else {
        for (var k = 0u; k <= r; k++) { acc += src[base + min(k, inner - 1u) * stride]; }
        acc += src[base] * f32(r);
        for (var i = 0u; i < inner; i++) {
