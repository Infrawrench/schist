...
fn point(x: u32, y: u32, sx: u32, sy: u32) -> vec2<f32> {
    let xi = 15u + x * 10u + sx * 2u;
    let yi = 15u + shape.width * 10u + y * 10u + sy * 2u;
...
fn fetch(x: i32, y: i32, c: u32) -> f32 {
    ...
    let xx = x - i32(args[6]);
    let yy = y - i32(args[7]);
    if shape.channels == 1u && (xx < 0 || yy < 0 || xx >= w || yy >= h) {
        return 0.0;
    }
    let j = u32(clamp(yy, 0, h - 1) * w + clamp(xx, 0, w - 1)) * shape.channels;
    if shape.channels == 1u || c == shape.channels - 1u {
        return src[j + c];
    }
...
fn sample_at(p: vec2<f32>, c: u32) -> f32 {
    ...
    if args[12] == 0.0 {
        return fetch(nearest(p.x), nearest(p.y), c);
    }
    ...
    if args[12] == 2.0 {
        ...
        for (var y = 0u; y < 4u; y++) {
            for (var x = 0u; x < 4u; x++) {
                let weight = wx[x] * wy[y];
                if weight != 0.0 {
                    sum += fetch(base.x + i32(x) - 1, base.y + i32(y) - 1, c) * weight;
                }
            }
        }
    } else {
        for (var y = 0; y < 2; y++) {
            for (var x = 0; x < 2; x++) {
                ...
                if weight != 0.0 {
                    sum += fetch(base.x + x, base.y + y, c) * weight;
                }
            }
        }
    }
...
fn compute(i: u32) {
    ...
    if shape.channels == 1u {
        dst[i] = clamp(floor(sample_at(p, 0u) + 0.5), 0.0, 255.0);
        return;
    }
    var hits = 0u;
    for (var sy = 0u; sy < 4u; sy++) {
        for (var sx = 0u; sx < 4u; sx++) {
            ...
            if q.x >= args[6] && q.y >= args[7] && q.x < args[6] + args[8] && q.y < args[7] + args[9] {
                hits++;
            }
        }
    }
