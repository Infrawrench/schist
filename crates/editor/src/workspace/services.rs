...
impl Workspace {
    /// Import user-trained weights without downloading or rebuilding Schist.
    pub fn import_neural_model(&mut self, id: &'static str, cx: &mut Context<Self>) {
        let Some(spec) = schist_neural::spec(id) else {
            return;
        };
        if spec.source != schist_neural::ModelSource::Local
            || self.model_downloads.iter().any(|d| d.id == id)
        {
            return;
        }
        let picker = self.prompt_for_paths(
            gpui::PathPromptOptions {
                ...
                directories: false,
                multiple: false,
                prompt: Some(schist_i18n::t("common.import").into()),
            },
            cx,
        );
        cx.spawn(async move |this, cx| {
            ...
            let started = this.update(cx, |ws, cx| {
                if ws.model_downloads.iter().any(|d| d.id == id) {
                    return false;
                }
                ws.model_downloads.push(ModelDownload {
                    id,
                    got: Arc::new(AtomicU64::new(0)),
                });
                ws.status = schist_i18n::t("common.loading").into();
                cx.notify();
                true
            });
            ...
            let result = crate::web::read_file(&path)
                .map_err(|e| anyhow::anyhow!("{e:?}"))
                .and_then(|bytes| schist_neural::install_local(spec, &bytes));
            ...
            let result = cx
                ...
                .spawn(async move {
                    ...
                    std::fs::File::open(path)?
                        .take(schist_neural::MAX_LOCAL_MODEL_BYTES as u64 + 1)
                        .read_to_end(&mut bytes)?;
                    schist_neural::install_local(spec, &bytes)
                })
                .await;
            this.update(cx, |ws, cx| {
                ...
                ws.status = match result {
                    ...
                    .into(),
                    Err(error) => {
                        log::warn!("model import failed: {error:#}");
                        schist_i18n::t("common.failed").into()
                    }
                };
                cx.notify();
            })
