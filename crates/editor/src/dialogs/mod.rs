...
pub fn render(ws: &mut Workspace, cx: &mut Context<Workspace>) -> Option<gpui::AnyElement> {
    ...
    let body = match modal {
        ...
        Modal::NewFilePicker {
            ...
            importing,
            error,
        } => new_file_picker(&clipboard, importing, error, cx).into_any_element(),
        ...
        #[cfg(not(target_arch = "wasm32"))]
        Modal::VariantName { name, rename, .. } => {
            crate::workspace::library_variants::variant_name_dialog(ws, name, rename, cx)
                .into_any_element()
        }
        #[cfg(target_arch = "wasm32")]
        ...
        Modal::BucketName {
            ...
            cloud,
            ..
        } => crate::workspace::bucket_name_dialog(
            ws,
            crate::workspace::BucketDialogText {
                name,
                query,
                exclude_query,
            },
