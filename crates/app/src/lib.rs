...
/// Runs the app: the body of the executable's `main`, and of Android's
/// `android_main`.
pub fn main() {
    #[cfg(target_os = "android")]
    android::prepare_environment();
    #[cfg(not(target_arch = "wasm32"))]
    ...
    // On Android this waits on its own thread: opening the compositor's
    // Vulkan device and compiling its kernels can take a while (minutes on
    // the emulator's software Vulkan), and the activity's main thread is
    // blocked until `main` reaches gpui's loop, which Android reads as
    // the app not responding. The CPU compositor serves until the GPU one
    // is installed; the backend swap is made for that (the Preferences
    // toggle does the same at runtime).
    #[cfg(target_os = "android")]
    {
        let prefer_gpu = options.gpu_compositing;
        std::thread::Builder::new()
            .name("gpu-compositor-init".into())
            .spawn(move || workspace::init_compositor_backend(prefer_gpu))
            .ok();
    }
...
// The Android entry point: NativeActivity calls `android_main`, which runs
// `main` above. Expands to nothing on every other platform.
