        ...
        unsafe {
            loop {
                ...
                if let Ok(buffer) = buffer.cast::<IMF2DBuffer>() {
                    ...
                    copy_rows(base, stride, width, height, &mut rgba)?;
                    drop(locked);
                } else {
...
unsafe fn copy_rows(
    ...
    rgba: &mut Vec<u8>,
        ...
        !base.is_null() && stride.unsigned_abs() as u64 >= width as u64 * 4,
        "{}",
        t("video.decode_failed")
    );
    for y in 0..height as isize {
        let row = std::slice::from_raw_parts(base.offset(y * stride as isize), width as usize * 4);
