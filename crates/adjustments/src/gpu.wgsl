...
fn apply_direct(kind: u32, base: u32, c: vec3<f32>) -> vec3<f32> {
    switch kind {
        ...
        case 13u: {
            return vec3(adj_arg(base), adj_arg(base + 1u), adj_arg(base + 2u));
        }
        case 14u: {
            return clamp(
                pow(
                    max(c * adj_arg(base) + vec3(adj_arg(base + 1u)), vec3(0.0)),
                    vec3(adj_arg(base + 2u))
                ),
                vec3(0.0),
                vec3(1.0)
            );
        ...
        case 15u: {
            return clamp(
                ((c + vec3(adj_arg(base))) - vec3(0.5)) * adj_arg(base + 1u) + vec3(0.5),
                vec3(0.0),
                vec3(1.0)
            );
        }
        case 16u: {
            var out = c;
            for (var k = 0u; k < 3u; k++) {
                out[k] = adj_level(base, adj_level(base + 5u * (k + 1u), c[k]));
            }
            return out;
        }
        case 17u: {
            var out = c;
            for (var k = 0u; k < 3u; k++) {
                out[k] = adj_curve(base + u32(adj_arg(base)), adj_curve(base + u32(adj_arg(base + k + 1u)), c[k]));
            }
            return out;
        }
        case 18u: {
        ...
        case D_COLOR_BALANCE: {
            ...
            if adj_arg(base + 9u) != 0.0 {
                return adj_set_lum(out, lum);
            }
            return out;
        }
        case D_VIBRANCE: {
            return adj_vibrance(base, c);
        }
        case D_PHOTO_FILTER: {
            let k = adj_arg(base + 3u);
            let out = clamp(c * (vec3(1.0 - k) + k * adj_vec3(base)), vec3(0.0), vec3(1.0));
            if adj_arg(base + 4u) != 0.0 {
                return adj_set_lum(out, adj_luma(c));
            }
            return out;
        }
        case D_GRADIENT_MAP: {
            ...
            if count < 2u {
                return adj_vec3(base + 2u) + (adj_vec3(base + 5u) - adj_vec3(base + 2u)) * t;
            }
            var lo = base + 8u;
            var hi = lo + (count - 1u) * 4u;
            for (var i = 0u; i + 1u < count; i++) {
                let a = base + 8u + i * 4u;
                if t >= adj_arg(a) && t <= adj_arg(a + 4u) {
                    lo = a;
                    hi = a + 4u;
                    break;
                }
        ...
        case D_SELECTIVE_COLOR: {
            return adj_selective(base, c);
        }
        case D_CHANNEL_MIXER: {
            ...
            for (var i = 0u; i < 3u; i++) {
                ...
                out[i] = clamp(
                    (c.r * v.x + c.g * v.y + c.b * v.z) / 100.0 + adj_arg(base + 10u + row) / 100.0,
                    0.0,
                    1.0
                );
            }
...
fn adj_hue_weight(base: u32, hue: f32) -> f32 {
    ...
    if down > flat {
        return (down - d) / (down - flat);
    }
    return 1.0;
}

fn adj_luma(c: vec3<f32>) -> f32 {
...
fn adj_set_lum(rgb: vec3<f32>, desired: f32) -> vec3<f32> {
    ...
    if lo < 0.0 && l - lo > 0.000001 {
        c = vec3(l) + (c - vec3(l)) * l / (l - lo);
    }
    let hi = max(c.r, max(c.g, c.b));
    if hi > 1.0 && hi - l > 0.000001 {
        c = vec3(l) + (c - vec3(l)) * (1.0 - l) / (hi - l);
    }
...
fn adj_vibrance_boost(base: u32, chroma: f32) -> f32 {
    ...
    if chroma <= 2.5 {
        return adj_arg(base + 4u) * (chroma / 2.5);
    }
    let x = (chroma - 2.5) / 5.0;
    let i = u32(floor(x)) + 1u;
    if i + 1u >= 20u {
        return adj_arg(base + 22u);
    }
    let f = x - floor(x);
...
fn adj_vibrance(base: u32, c: vec3<f32>) -> vec3<f32> {
    ...
    if t > 0.0 {
        ...
        var protect = 0.0;
        if hue < 30.0 {
            protect = clamp((30.0 - hue) / 45.0, 0.0, 1.0);
        } else if hue > 45.0 {
            protect = clamp((hue - 45.0) / 45.0, 0.0, 1.0);
        }
...
fn adj_selective(base: u32, c: vec3<f32>) -> vec3<f32> {
    ...
    if mx == c.r {
        primary = 0u;
        secondary = select(5u, 1u, c.g >= c.b);
    } else if mx == c.g {
        primary = 2u;
        secondary = select(1u, 3u, c.b >= c.r);
    } else if c.r >= c.g {
        secondary = 5u;
    }
...
fn adj_level(base: u32, value: f32) -> f32 {
    ...
    if adj_arg(base + 2u) != 1.0 {
        t = pow(t, adj_arg(base + 2u));
    }
    return clamp(adj_arg(base + 3u) + t * adj_arg(base + 4u), 0.0, 1.0);
}

fn adj_curve(base: u32, value: f32) -> f32 {
    let n = u32(adj_arg(base));
    if n == 0u {
        return value;
    }
    let x = clamp(value, 0.0, 1.0);
    if x <= adj_arg(base + 1u) {
        return clamp(adj_arg(base + 2u), 0.0, 1.0);
    }
    for (var i = 0u; i + 1u < n; i++) {
        ...
        let prev = adj_arg(base + 2u + u32(max(i32(i) - 1, 0)) * 2u);
        let next = adj_arg(base + 2u + min(i + 2u, n - 1u) * 2u);
        let t2 = t * t;
        let t3 = t2 * t;
        return clamp(
            0.5 * ((2.0 * y0) + (-prev + y1) * t
                + (2.0 * prev - 5.0 * y0 + 4.0 * y1 - next) * t2
                + (-prev + 3.0 * y0 - 3.0 * y1 + next) * t3),
            0.0,
            1.0
        );
    }
